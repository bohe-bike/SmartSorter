use crate::models::rule::{Condition, ConditionField, ConditionGroup, Logic, Operator};
...
pub fn matches(path: &Path, group: &ConditionGroup) -> bool {
    // 每个文件只读一次 metadata，传递给每个条件评估
    let meta = fs::metadata(path).ok();
    matches_with_meta(path, group, meta.as_ref())
}

fn matches_with_meta(path: &Path, group: &ConditionGroup, meta: Option<&fs::Metadata>) -> bool {
    let cond_results = group
        .conditions
        .iter()
        .map(|c| evaluate_condition(path, c, meta));
    let sub_results = group
        .sub_groups
        .iter()
        .map(|sg| matches_with_meta(path, sg, meta));
...
fn evaluate_condition(path: &Path, cond: &Condition, meta: Option<&fs::Metadata>) -> bool {
    match cond.field {
        ...
        ConditionField::CreatedAt => meta
            .and_then(|m| m.created().ok())
            .map(|t| match_timestamp(t, &cond.operator, &cond.value))
            .unwrap_or(false),
        ConditionField::ModifiedAt => meta
...
fn match_timestamp(value: SystemTime, op: &Operator, target: &serde_json::Value) -> bool {
    match op {
        Operator::Before => parse_datetime(target)
            .map(|t| DateTime::<Local>::from(value) < t)
            .unwrap_or(false),
        Operator::After => parse_datetime(target)
            .map(|t| DateTime::<Local>::from(value) > t)
            .unwrap_or(false),
        Operator::WithinDays => target
            .as_u64()
            .map(|days| {
